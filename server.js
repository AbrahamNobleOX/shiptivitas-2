...
app.put("/api/v1/clients/:id", (req, res) => {
  ...
  // let clientsOld = db.prepare("select * from clients WHERE id = ?").get(id);
  // let clients = db
  //   .prepare("UPDATE clients SET name = 'Wiza LLC' WHERE id = ?")
  //   .run(id);
  // let clientsNew = db.prepare("select * from clients WHERE id = ?").get(id);
  ...
  // return res
  //   .status(200)
  //   .send({ clientsOld, status, clients, priority, clientsNew });
  ...
  if (oldStatus === newStatus && priority && oldPriority !== priority) {
    const clientsWithDifferentStatus = clients.filter(
      (client) => client.status !== newStatus
    );
    client.priority = priority - 0.5;
    const clientsWithSameStatus = clients
    ...
    clients = [...clientsWithDifferentStatus, ...clientsWithSameStatus];
